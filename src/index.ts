...
export function apply(ctx: Context, config: Config) {
  ...
  async function renderStockImage(
    ctx: Context,
    data: { time: string; price: number; timestamp: number }[],
    name: string,
    viewLabel: string,
    current: number,
    high: number,
    ...
    try {
      ...
      const klineData =
        chartType === "kline"
          ? buildKLineData(data, klineBucketMs ?? 2 * 3600 * 1000)
          : [];
  ...
  function buildKLineData(
    ...
    let lastClose = data[0].price;

    for (let i = 0; i < candleCount; i++) {
      const bucketStart = startTs + i * bucketMs;
      const bucketEnd = bucketStart + bucketMs;
      const bucket = data.filter((d) => {
        if (i === candleCount - 1)
          return d.timestamp >= bucketStart && d.timestamp <= bucketEnd;
        return d.timestamp >= bucketStart && d.timestamp < bucketEnd;
      });
      ...
      if (bucket.length === 0) {
        candles.push({
          ...
          start: Number(lastClose.toFixed(2)),
          max: Number(lastClose.toFixed(2)),
          min: Number(lastClose.toFixed(2)),
          end: Number(lastClose.toFixed(2)),
          timestamp: bucketEnd,
        });
        continue;
      }
