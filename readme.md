...
- **💥 熔断机制**：模拟真实股市的熔断保护机制。当日跌幅超过配置阈值时自动触发熔断，暂停交易并冻结价格。支持配置触发阈值、持续时间和后期望模式（回弹/随机/持平）。管理员可通过 `stock.circuit` 命令查看或手动清除熔断状态。
- **💱 买卖价差**：模拟真实市场的买卖价差。卖出时实际成交价低于市价，买入时实际成交价高于市价，增加交易的真实感和博弈深度。
...
- **`stock.buy <amount>`**
  - 买入股票。
  - 参数 `amount`: 购买股数（整数）或 `all`（全部买入）。
  - 说明：扣除现金（优先）或银行活期，股票将在冻结时间结束后到账。使用 `all` 时自动计算最大可买股数。
  - _新特性：若无冻结时间（小额或配置设置），将直接返回一张包含买入点位标记的**交易交割单图片**。_
- **`stock.sell <amount>`**
  - 卖出股票。
  - 参数 `amount`: 卖出股数（整数）或 `all`（全部卖出）。
  - 说明：扣除持仓，获得的资金将在冻结时间结束后到账。使用 `all` 时卖出全部持仓。
...
- **`stock.circuit [action]`** _(3.1.0 新增)_
  - 查看或清除熔断状态。
  - 参数 `action`: 可选 `clear`（清除当前熔断状态）。
  - 说明：无参数时显示熔断机制配置和当前状态；传入 `clear` 可手动解除熔断。
